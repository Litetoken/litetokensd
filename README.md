# Description
Litetokens is a protocol for the creation and use of decentralised financial
...
repository, `litetokensd`, constitute the reference implementation of the
protocol.

The Litetokens protocol specification may be found at
<https://github.com/Litetokens/Litetokens>.

# Dependencies
* [Python 3](http://python.org)
* Python 3 packages: apsw, requests, appdirs, prettytable, python-dateutil, json-rpc, tornado, flask, Flask-HTTPAuth, pycoin, pyzmq(v2.2+), pycrypto, lockfile, python-bitcoinlib (see [this link](https://github.com/LitetokensXLT/litetokensd/blob/master/pip-requirements.txt) for exact working versions)
* Litecoind

# Installation

**NOTE: This section covers manual installation of litetokensd. If you want more of
an automated approach to litetokensd installation for Windows and Linux, see [this link](http://litetokens.io/docs/build-system/).**

In order for litetokensd to function, it must be able to communicate with a
...
litetokensd needs to know at least the JSON‐RPC password of the Litecoind with
which it is supposed to communicate. The simplest way to set this is to
include it in all command‐line invocations of litetokensd, such as
`./litetokensd.py --rpc-password=PASSWORD ACTION`. To make this and other
options persistent across litetokensd sessions, one may store the desired
settings in a configuration file specific to litetokensd.
...
However, a litetokensd configuration file looks like this:
...
If and only if litetokensd is to be run on the Litecoin testnet, with the
`--testnet` CLI option, Litecoind must be set to do the same (`-testnet=1`).
litetokensd may run with the `--testcoin` option on any blockchain,
however.

# Updating your requirements

Sometimes the underlying package requirements may change for `litetokensd`. If you build and installed it from scratch,
...
Do not include the following values ​​in litetokensd.conf: bitcoind-rpc-connect, bitcoind-rpc-port, rpc-host, rpc-port and testnet.

# Usage
The command‐line syntax of litetokensd is generally that of
`./litetokensd.py {OPTIONS} ACTION {ACTION-OPTIONS}`. There is a one action
...
provide information about the state of the Litetokens network, e.g. current
...
* litetokensd identifies an Order, Bet, Order Match or Bet Match by an
...
* Buy LTC for XLT
	...
	--give-quantity=20 --give-asset=XLT --expiration=10 --fee_required=.001
	```
...
* Buy XLT for BBBC
	```
	order --source=mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns --get-quantity=10 --get-asset=XLT
...
* Broadcast
	...
	Note: for some users litetokensd has trouble parsing spaces in the `--text` argument. One workaround is to
...
* Bet
	...
	Example: Bet on Super Bowl Feed. Denver vs. Seattle. Feed value of 1 means Seattle Wins. Feed value of 2 means 	        	Denver Wins. This command places a 1 XLT bet on the Super Bowl Feed for Seattle to win, paying out 2 to         	1. The bet will expire in 100 blocks and the settlement value of the bet is based on the first feed 	                update after the deadline timestamp of February 3, 2014 1:39 PM US Eastern Standard Time (UTC-0500)
	```
	...
	Example: Bet on Litecoin Price Feed. This command places a bearish (short) 1 XLT wager on the price of LTC/USD 				with 2X leverage. The bet will expire in 100 blocks and the settlement value of the bet is based 			on the first feed update after the deadline timestamp of February 3, 2014 1:39 PM US Eastern 					Standard Time (UTC-0500)
	```
...
* Market
	...
	To filter the market to only show offers to sell LTC for XLT:
	```
	market --give-asset=LTC --get-asset=XLT
	```
...
* Address
	...
	The `address` action displays the details of of all transactions involving the Litetokens address which is its argument.

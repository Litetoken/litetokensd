...
``litetokensd`` features a full-fledged JSON RPC 2.0-based API, which allows
third-party applications to perform functions on the Litetokens network
...
By default, ``litetokensd`` will listen on port ``4000`` (if on mainnet) or port ``14000`` (on testnet) for API
requests. 
...
Authentication
^^^^^^^^^^^^^^^
Also note that the ``litetokensd`` API interface requires HTTP basic authentication to use. The username and password required
are stored in the ``litetokensd.conf`` file, as ``rpc-user`` and ``rpc-password``, respectively. You can also modify
``rpc-host`` and ``rpc-port`` to change what interface and port number ``litetokensd`` binds to from the defaults.

.. _examples:

Below we provide a few examples of using the ``litetokensd`` API. Examples in other languages are welcome,
...
.. code-block:: python
    ...
    #Get all burns between blocks 280537 and 280539 where greater than .2 LTC was burned, sorting by tx_hash (ascending order)
    ...
    #Fetch all debits for > 2 XLT between blocks 280537 and 280539, sorting the results by quantity (descending order)
    payload = {
      "method": "get_debits",
      "params": {"filters": [{'field': 'asset', 'op': '==', 'value': "XLT"},
    ...
    #Send 1 XLT (specified in satoshis) from one address to another (you must have the sending address in your litecoind wallet
...
With PHP, you can connect and query ``litetokensd`` using the `JsonRPC <https://github.com/fguillot/JsonRPC>`__
...
26) string name of the asset, of at least 4 characters in length and not starting with 'A', or as 'LTC' or 'XLT' as appropriate. Examples are:
...
**NOTE:** XLT and LTC themselves are divisible assets, and thus are listed in satoshis.
...
The Litetokens API aims to be as simple and flexible as possible. To this end, it includes a straightforward
...
All ``create_`` API calls return an *unsigned raw transaction string*, hex encoded (i.e. the same format that ``litecoind`` returns
...
- To return the transaction as an **OP_RETURN** transaction, specify ``opreturn`` for the ``encoding`` parameter.
  Note that as of ``litecoind`` 0.9.0, not all Litetokens transactions are possible with OP_RETURN, due to the 40
...
- To return the transaction as a **multisig** transaction, specify ``multisig`` for the ``encoding`` parameter.
    ...
    - If the source address is *not* in the local ``litecoind`` ``wallet.dat``, ``pubkey`` should be set to the hex-encoded
      public key.
- ``auto`` may also be specified to let ``litetokensd`` choose here. Note that at this time, ``auto`` is effectively the same as
  ``multisig``.

- To return the Litetokens transaction encoded into arbitrary address outputs (i.e. pubkeyhash encoding), specify
...
- If the private key you need to sign the raw transaction is in the local ``litecoind`` ``wallet.dat``, you should then call the
...
- If the private key you need to sign the raw transaction is *not* in the local ``litecoind`` ``wallet.dat``, you must first sign
  ...
  the private key string to it, and ``litetokensd`` will sign it for you). In either case, once you have the signed,
...
``balances``, ``credits``, ``debits``, ``bets``, ``bet_matches``, ``broadcasts``, ``ltcpays``, ``burns``, 
...
**Notes:**
  ...
  * Please note that the ``get_balances`` API call will not return balances for LTC itself. It only returns balances
    for XLT and other Litetokens assets. To get LTC-based balances, use an existing system such as Insight, blockr.io,
    or blockchain.info.
...
Returns a list of all existing Litetokens assets. 

**Parameters:** None

**Return:**

  A list of existing Litetokens asset names.
...
Return message feed activity for the specified block index. The message feed essentially tracks all litetokensd
...
Gets the current total quantity of XLT in existance (i.e. quantity created via proof-of-burn, minus quantity
...
**Return:** 
  ...
  The :ref:`quantity <quantitys>` of XLT currently in existance.
...
Gets some operational parameters for litetokensd.
...
**Return:** 
  ...
  - **db_caught_up** (*boolean*): ``true`` if litetokensd block processing is caught up with the Litecoin blockchain, ``false`` otherwise.
  - **litecoin_block_count** (**integer**): The block height on the Litecoin network (may not necessarily be the same as ``last_block``, if ``litetokensd`` is catching up)
  - **last_block** (*integer*): The index (height) of the last block processed by ``litetokensd``
  - **litetokensd_version** (*float*): The litetokensd program version, expressed as a float, such as 0.5
  - **last_message_index** (*integer*): The index (ID) of the last message in the ``litetokensd`` message feed
  - **running_testnet** (*boolean*): ``true`` if litetokensd is configured for testnet, ``false`` if configured on mainnet.
  - **db_version_major** (*integer*): The major version of the current litetokensd database
  - **db_version_minor** (*integer*): The minor version of the current litetokensd database
...
**Parameters:**
  ...
  * **privkey (string):** The private key in WIF format to use for signing the transaction. If not provided,
    the private key must to be known by the ``litecoind`` wallet.
...
Broadcast a signed transaction onto the Litecoin network.
...
**Return:** 
  ...
  The created transaction's id on the Litecoin network, or an error if the transaction is invalid for any reason.
...
**Parameters:**
  ...
  * **wager (integer):** The :ref:`quantity <quantitys>` of XLT to wager.
  * **counterwager (integer):** The minimum :ref:`quantity <quantitys>` of XLT to be wagered against, for the bets to match.
  ...
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``litetokensd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``litetokensd`` to automatically choose. 
  * **fee_per_kb (integer):** The fee per kilobyte of transaction data constant that ``litetokensd`` uses when deciding on the dynamic fee to use (in satoshi). Leave as default unless you know what you're doing.
...
create_ltcpay
^^^^^^^^^^^^^^
**create_ltcpay(order_match_id, encoding='multisig', pubkey=null,
allow_unconfirmed_inputs=false, fee=null, fee_per_kb=10000)**

Create and (optionally) broadcast a LTCpay message, to settle an Order Match for which you owe LTC. 
...
Burn a given quantity of LTC for XLT (**only possible between blocks 278310 and 283810**).
...
**Parameters:**
  ...
  * **quantity_per_unit (integer):** The :ref:`quantity <quantitys>` of XLT rewarded per whole unit of the asset.
  * **encoding (string):** The encoding method to use, see :ref:`this section <encoding_param>` for more info.  
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``litetokensd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``litetokensd`` to automatically choose. 
...
**Parameters:**
  ...
  * **call_price (float):** The :ref:`price <floats>` per unit XLT at which the asset may be called back, on or after the specified call_date. Only valid for callable assets.
...
Send XLT or a user defined asset.
...
``bet``, ``broadcast``, ``ltcpay``, ``burn``,  ``callback``, ``cancel``, ``dividend``, ``issuance``,
...
* **wager_quantity** (*integer*): The :ref:`quantity <quantitys>` of XLT to wager
* **counterwager_quantity** (*integer*): The minimum :ref:`quantity <quantitys>` of XLT to be wagered by the user to bet against the bet issuer, if the other party were to accept the whole thing
* **wager_remaining** (*integer*): The quantity of XLT wagered that is remaining to bet on
* **odds** (*float*): 
* **target_value** (*float*): Target value for Equal/NotEqual bet
* **leverage** (*integer*): Leverage, as a fraction of 5040
...
* **tx0_index** (*integer*): The Litecoin transaction index of the initial bet
* **tx0_hash** (*string*): The Litecoin transaction hash of the initial bet
* **tx0_block_index** (*integer*): The Litecoin block index of the initial bet
...
* **target_value** (*float*): Target value for Equal/NotEqual bet  
* **leverage** (*integer*): Leverage, as a fraction of 5040
* **forward_quantity** (*integer*): The :ref:`quantity <quantitys>` of XLT bet in the initial bet
* **backward_quantity** (*integer*): The :ref:`quantity <quantitys>` of XLT bet in the matching bet
* **fee_multiplier** (*integer*): 
...
An object that matches a request to settle an Order Match for which LTC is owed:
...
* **burned** (*integer*): The :ref:`quantity <quantitys>` of LTC burned
* **earned** (*integer*): The :ref:`quantity <quantitys>` of XPC actually earned from the burn (takes into account any bonus quantitys, 1 LTC limitation, etc)
...
* **fee_provided** (*integer*): The miners' fee provided; in LTC; required only if selling LTC (should not be lower than is required for acceptance in a block)
* **fee_required** (*integer*): The miners' fee required to be paid by orders for them to match this one; in LTC; required only if buying LTC (may be zero, though)
...
* **tx0_index** (*integer*): The Litecoin transaction index of the first (earlier) order
* **tx0_hash** (*string*): The Litecoin transaction hash of the first order
* **tx0_block_index** (*integer*): The Litecoin block index of the first order
...
An object that describes a specific send (e.g. "simple send", of XLT, or a user defined asset):
...
An object that describes a specific event in the litetokensd message feed (which can be used by 3rd party applications
to track state changes to the litetokensd database on a block-by-block basis).
...
* **category** (*string*): A string denoting the entity that the message relates to, e.g. "credits", "burns", "debits".
  The category matches the relevant table name in litetokensd (see blocks.py for more info).
...
* **ltcpays**: valid, invalid: {problem(s)}
...
This section documents any changes to the ``litetokensd`` API, for version numbers where there were API-level modifications.
...
* DatabaseError: now a DatabaseError is returned immediately if the litetokensd database is behind the backend, instead of after fourteen seconds

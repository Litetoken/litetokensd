...
def parse (db, tx, message):
    ...
    if status == 'valid':
        ...
        util.debit(db, tx['block_index'], tx['source'], config.XLT, int(call_price * callback_total), action='callback', event=tx['tx_hash'])
        ...
        for output in outputs:
            ...
            util.credit(db, tx['block_index'], output['address'], config.XLT, int(call_price * output['callback_quantity']), action='callback', event=tx['tx_hash'])

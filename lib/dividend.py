...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.XLT:
        if (not block_index >= 317500) or block_index >= 320000 or config.TESTNET:   # Protocol change.
            problems.append('cannot pay dividends to holders of {}'.format(config.XLT))
    ...
    if dividend_asset in (config.LTC, config.XLT):
    ...
    if not problems and dividend_asset != config.LTC:
        ...
        if fee:
            balances = list(cursor.execute('''SELECT * FROM balances WHERE (address = ? AND asset = ?)''', (source, config.XLT)))
            if not balances or balances[0]['quantity'] < fee:
                problems.append('insufficient funds ({})'.format(config.XLT))
...
def parse (db, tx, message):
    ...
    if status == 'valid':
        ...
        if tx['block_index'] >= 330000 or config.TESTNET: # Protocol change.
            util.debit(db, tx['block_index'], tx['source'], config.XLT, fee, action='dividend fee', event=tx['tx_hash'])

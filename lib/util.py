...
def api (method, params):
    ...
    if response == None:
        raise exceptions.RPCError('Cannot communicate with {} server.'.format(config.XLT_CLIENT))
        ...
        if category == 'credits':
            ...
            if bindings['transfer']:
                ...
                if bindings['callable'] and (bindings['block_index'] > 283271 or config.TESTNET):   # Protocol change.
                    callability = 'callable from {} for {} XLT/{}'.format(isodt(bindings['call_date']), bindings['call_price'], bindings['asset'])
                else:
            ...
            if 'CFD' not in BET_TYPE_NAME[bindings['bet_type']]:
                log_message = 'Bet: {} against {}, by {}, on {} that ‘{}’ will {} {} at {}, {}'.format(output(bindings['wager_quantity'], config.XLT), output(bindings['counterwager_quantity'], config.XLT), bindings['source'], bindings['feed_address'], text, BET_TYPE_NAME[bindings['bet_type']], str(output(bindings['target_value'], 'value').split(' ')[0]), isodt(bindings['deadline']), end)
            else:
                log_message = 'Bet: {}, by {}, on {} for {} against {}, leveraged {}x, {}'.format(BET_TYPE_NAME[bindings['bet_type']], bindings['source'], bindings['feed_address'],output(bindings['wager_quantity'], config.XLT), output(bindings['counterwager_quantity'], config.XLT), output(bindings['leverage']/ 5040, 'leverage'), end)
            ...
            logging.info('Bet Match: {} for {} against {} for {} on {} at {}{} ({}) [{}]'.format(BET_TYPE_NAME[bindings['tx0_bet_type']], output(bindings['forward_quantity'], config.XLT), BET_TYPE_NAME[bindings['tx1_bet_type']], output(bindings['backward_quantity'], config.XLT), bindings['feed_address'], isodt(bindings['deadline']), placeholder, bindings['id'], bindings['status']))
            ...
            logging.info('Burn: {} burned {} for {} ({}) [{}]'.format(bindings['source'], output(bindings['burned'], config.LTC), output(bindings['earned'], config.XLT), bindings['tx_hash'], bindings['status']))
            ...
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], 'XLT'))
            logging.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], 'XLT'), bindings['id'], bindings['status'])
            logging.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], 'XLT'), rps_match['id'], rps_match['status'])
            else:
            ...
            if bindings['bet_match_type_id'] == cfd_type_id:
                if bindings['settled']:
                    logging.info('Bet Match Settled: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.XLT), output(bindings['bear_credit'], config.XLT), output(bindings['fee'], config.XLT), bindings['bet_match_id']))
                else:
                    logging.info('Bet Match Force‐Liquidated: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.XLT), output(bindings['bear_credit'], config.XLT), output(bindings['fee'], config.XLT), bindings['bet_match_id']))

            elif bindings['bet_match_type_id'] == equal_type_id:
                logging.info('Bet Match Settled: {} won the pot of {}; {} credited to the feed address ({})'.format(bindings['winner'], output(bindings['escrow_less_fee'], config.XLT), output(bindings['fee'], config.XLT), bindings['bet_match_id']))
...
def version_check (db):
    try:
        host = 'https://raw.githubusercontent.com/Litetokens/litetokensd/evolve/version.json'
    ...
    if not passed:
        explanation = 'Your version of litetokensd is v{}, but, as of block {}, the minimum version is v{}.{}.{}. Reason: ‘{}’. Please upgrade to the latest version and restart the server.'.format(
...
def database_check (db, blockcount):
    """Checks {} database to see if the {} server has caught up with Litecoind.""".format(config.XLT_NAME, config.XLT_CLIENT)
    if last_block(db)['block_index'] + 1 < blockcount:
        raise exceptions.DatabaseError('{} database is behind Litecoind. Is the {} server running?'.format(config.XLT_NAME, config.XLT_CLIENT))
    return
...
def asset_id (asset):
    ...
    elif asset == config.XLT: return 1
...
def asset_name (asset_id):
    ...
    elif asset_id == 1: return config.XLT
...
def debit (db, block_index, address, asset, quantity, action=None, event=None):
    ...
    if asset == config.LTC:
        raise exceptions.BalanceError('Cannot debit litecoins from a {} address!'.format(config.XLT_NAME))
...
def holders(db, asset):
    ...
    # Bets and RPS (and bet/rps matches) only escrow XLT.
    if asset == config.XLT:
...
def supplies (db):
    cursor = db.cursor()
    supplies = {config.XLT: xlt_supply(db)}
    cursor.execute('''SELECT * from issuances \
                      WHERE status = ?''', ('valid',))
